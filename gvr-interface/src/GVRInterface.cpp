...
JNIEXPORT void Java_io_highfidelity_gvrinterface_InterfaceActivity_handleHifiURL(JNIEnv* jni, jclass clazz,
                                                                                 jstring hifiURLString) {
...
GVRInterface::GVRInterface(int argc, char* argv[]) : QApplication(argc, argv), _mainWindow(NULL), _inVRMode(false) {
...
void GVRInterface::idle() {
    ...
    if (!_inVRMode && ovr_IsHeadsetDocked()) {
        ...
        if (ovr_IsHeadsetDocked()) {
            ...
            if (counter++ % 100000 == 0) {
                qDebug() << "GetSensorState in frame" << counter << "-" << ovrOrientation.x << ovrOrientation.y
                         << ovrOrientation.z << ovrOrientation.w;
            }
        } else {
