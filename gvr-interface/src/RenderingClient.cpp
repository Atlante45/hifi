...
RenderingClient::RenderingClient(QObject* parent, const QString& launchURLString) : Client(parent) {
    _instance = this;
    ...
    DependencyManager::get<NodeList>()->addSetOfNodeTypesToNodeInterestSet(NodeSet()
    ...
    switch (incomingType) {
        ...
        case PacketTypeAvatarBillboard: {
            ...
            if (avatarMixer) {
                ...
                QMetaObject::invokeMethod(DependencyManager::get<AvatarHashMap>().data(), "processAvatarMixerDatagram",
...
void RenderingClient::goToLocation(const glm::vec3& newPosition, bool hasOrientationChange, const glm::quat& newOrientation,
                                   bool shouldFaceLocation) {
    qDebug().nospace() << "RenderingClient goToLocation - moving to " << newPosition.x << ", " << newPosition.y << ", "
                       << newPosition.z;

    glm::vec3 shiftedPosition = newPosition;

    if (hasOrientationChange) {
        qDebug().nospace() << "RenderingClient goToLocation - new orientation is " << newOrientation.x << ", "
