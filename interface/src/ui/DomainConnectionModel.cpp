...
DomainConnectionModel::DomainConnectionModel(QAbstractItemModel* parent) : QAbstractItemModel(parent) {
}
...
QVariant DomainConnectionModel::data(const QModelIndex& index, int role) const {
    // sanity
    const QMap<quint64, LimitedNodeList::ConnectionStep>& times = DependencyManager::get<NodeList>()->getLastConnectionTimes();
