...
class SecondaryCameraJobConfig : public render::Task::Config { // Exposes secondary camera parameters to JavaScript.
    ...
    Q_PROPERTY(QUuid portalEntranceEntityId MEMBER portalEntranceEntityId NOTIFY
                   dirty) // entity whose properties define a portal's entrance position and orientation
    ...
    Q_PROPERTY(
        bool portalProjection MEMBER portalProjection NOTIFY dirty) // Flag to use attached portal entity to build frustum for
                                                                    // the portal and set portal camera position/orientation.
public:
