...
void MyAvatar::handleChangedAvatarEntityData() {
    ...
    for (const auto& id : entitiesToUpdate) {
        ...
        if (!skip) {
            sanitizeAvatarEntityProperties(properties);
            entityTree->withWriteLock([&] { entityTree->updateEntity(id, properties); });
        }
    }
...
static float lookAtCostFunction(const glm::vec3& myForward, const glm::vec3& myPosition, const glm::vec3& otherForward,
                                const glm::vec3& otherPosition, bool otherIsTalking, bool lookingAtOtherAlready) {
...
void MyAvatar::computeMyLookAtTarget(const AvatarHash& hash) {
    ...
    foreach (const AvatarSharedPointer& avatarData, hash) {
        ...
        if (!avatar->isMyAvatar() && avatar->isInitialized()) {
            ...
            float cost = lookAtCostFunction(myForward, myPosition, otherForward, otherPosition, otherIsTalking,
                                            lookingAtOtherAlready);
            if (cost < bestCost) {
                bestCost = cost;
                bestAvatar = avatar;
            }
...
bool MyAvatar::requiresSafeLanding(const glm::vec3& positionIn, glm::vec3& betterPositionOut) {
    ...
    auto findIntersection = [&](const glm::vec3& startPointIn, const glm::vec3& directionIn, glm::vec3& intersectionOut,
        ...
        EntityItemID entityID = entityTree->evalRayIntersection(startPointIn, directionIn, include, ignore,
                                                                PickFilter(
                                                                    PickFilter::getBitMask(PickFilter::FlagBit::COLLIDABLE) |
                                                                    PickFilter::getBitMask(PickFilter::FlagBit::PRECISE)),
                                                                element, distance, face, normalOut, extraInfo, lockType,
                                                                accurateResult);
        if (entityID.isNull()) {
            return false;
        }
...
const QUuid MyAvatar::grab(const QUuid& targetID, int parentJointIndex, glm::vec3 positionalOffset,
...
void MyAvatar::useFlow(bool isActive, bool isCollidable, const QVariantMap& physicsConfig,
