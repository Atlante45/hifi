...
/**jsdoc
 * The Pointers API lets you create and manage objects for repeatedly calculating intersections in different ways, as well as
 * the visual representation of those objects. Pointers can also be configured to automatically generate {@link PointerEvent}s
 * on {@link Entities}.
 *
 * @namespace Pointers
 *
...
public:
     ...
     * @property {Controller.Standard|Controller.Actions|function} action This can be a built-in Controller action, like
     * Controller.Standard.LTClick, or a function that evaluates to >= 1.0 when you want to trigger <code>button</code>.
     * @property {string} button Which button to trigger.  "Primary", "Secondary", "Tertiary", and "Focus" are currently
     * supported.  Only "Primary" will trigger clicks on web surfaces.  If "Focus" is triggered, it will try to set the entity
     * focus to the object at which the Pointer is aimed.  Buttons besides the first three will still trigger events, but
     * event.button will be "None".
     */
     ...
     * create a Static Ray Pointer, a Mouse Ray Pointer, or a Joint Ray Pointer. Pointers created with this method always
     * intersect at least visible and collidable things
     * @function Pointers.createPointer
     * @param {PickType} type A PickType that specifies the method of picking to use
     * @param {Pointers.LaserPointerProperties|Pointers.StylusPointerProperties|Pointers.ParabolaPointerProperties} properties A
     * PointerProperties object, containing all the properties for initializing this Pointer <b>and</b> the {@link
     * Picks.PickProperties} for the Pick that this Pointer will use to do its picking.
     * @returns {number} The ID of the created Pointer.  Used for managing the Pointer.  0 if invalid.
     *
     * @example <caption>Create a left hand Ray Pointer that triggers events on left controller trigger click and changes color
     * when it's intersecting something.</caption>
     *
     * var end = {
     *     type: "sphere",
     ...
     *     triggers: [ {action: Controller.Standard.LTClick, button: "Focus"}, {action: Controller.Standard.LTClick, button:
     * "Primary"} ], hover: true, enabled: true
     * });
    ...
    /**jsdoc
     * Set the render state of a Pointer.  For Ray Pointers, this means switching between their {@link
     * Pointers.RayPointerRenderState}s, or "" to turn off rendering and hover/trigger events. For Stylus Pointers, there are
     * three built-in options: "events on" (render and send events, the default), "events off" (render but don't send events),
     * and "disabled" (don't render, don't send events).
    ...
    /**jsdoc
     * Get the most recent pick result from this Pointer.  This will be updated as long as the Pointer is enabled, regardless of
     * the render state.
     * @function Pointers.getPrevPickResult
     * @param {number} uid The ID of the Pointer, as returned by {@link Pointers.createPointer}.
     * @returns {RayPickResult|StylusPickResult} The most recent intersection result.  This will be slightly different for
     * different PickTypes.
     */
     ...
     */
    Q_INVOKABLE void setLength(unsigned int uid, float length) const {
        DependencyManager::get<PointerManager>()->setLength(uid, length);
    }
    ...
    /**jsdoc
     * Lock a Pointer onto a specific object (entity or avatar).  Optionally, provide an offset in object-space, otherwise the
     * Pointer will lock on to the center of the object. Not used by Stylus Pointers.
     ...
     */
    Q_INVOKABLE void setLockEndUUID(unsigned int uid, const QUuid& objectID, bool isAvatar,
                                    const glm::mat4& offsetMat = glm::mat4()) const {
        DependencyManager::get<PointerManager>()->setLockEndUUID(uid, objectID, isAvatar, offsetMat);
    }
     ...
     * @param {number} uid The ID of the Pointer, as returned by {@link Pointers.createPointer}.
     * @returns {boolean} True if the Pointer is a Joint Ray Pointer with joint == "_CONTROLLER_LEFTHAND" or
     * "_CAMERA_RELATIVE_CONTROLLER_LEFTHAND", or a Stylus Pointer with hand == 0
     */
     ...
     * @param {number} uid The ID of the Pointer, as returned by {@link Pointers.createPointer}.
     * @returns {boolean} True if the Pointer is a Joint Ray Pointer with joint == "_CONTROLLER_RIGHTHAND" or
     * "_CAMERA_RELATIVE_CONTROLLER_RIGHTHAND", or a Stylus Pointer with hand == 1
     */
     ...
     * @param {number} uid The ID of the Pointer, as returned by {@link Pointers.createPointer}.
     * @returns {Pointers.LaserPointerProperties|Pointers.StylusPointerProperties|Pointers.ParabolaPointerProperties} The
     * information about the Pointer. Currently only includes renderStates and defaultRenderStates with associated entity IDs.
     */
