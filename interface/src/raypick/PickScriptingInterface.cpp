 ...
 * results.
 * @property {number} [filter=Picks.PICK_NOTHING] The filter for this Pick to use, constructed using filter flags combined using
 * bitwise OR.
    ...
    if (parentUuid == myAvatar->getSessionUUID()) {
        ...
        if (success && sharedNestablePointer) {
            ...
            if (nestableType == NestableType::Avatar) {
                ...
                pick->parentTransform = std::make_shared<OverlayTransformNode>(std::static_pointer_cast<Base3DOverlay>(
                                                                                   sharedNestablePointer),
                                                                               parentJointIndex);
