...
unsigned int PointerScriptingInterface::createPointer(const PickQuery::PickType& type, const QVariant& properties) {
    ...
    if (QThread::currentThread() != qApp->thread()) {
        unsigned int result;
        BLOCKING_INVOKE_METHOD(this, "createPointer", Q_RETURN_ARG(unsigned int, result), Q_ARG(PickQuery::PickType, type),
...
/**jsdoc
 * A set of properties that can be passed to {@link Pointers.createPointer} to create a new Pointer.  Contains the relevant
 * {@link Picks.PickProperties} to define the underlying Pick.
 ...
 * @property {Pointers.StylusPointerProperties.model} [model] Data to replace the default model url, positionOffset and
 * rotationOffset.
 */
/**jsdoc
...
unsigned int PointerScriptingInterface::createStylus(const QVariant& properties) const {
    ...
    return DependencyManager::get<PointerManager>()->addPointer(
        std::make_shared<StylusPointer>(properties, StylusPointer::buildStylus(propertyMap), hover, enabled,
                                        modelPositionOffset, modelRotationOffset, modelDimensions));
}

/**jsdoc
 * A set of properties used to define the visual aspect of a Ray Pointer in the case that the Pointer is not intersecting
 * something.  Same as a {@link Pointers.RayPointerRenderState}, but with an additional distance field.
 *
 ...
 * @property {string} name When using {@link Pointers.createPointer}, the name of this render state, used by {@link
 * Pointers.setRenderState} and {@link Pointers.editRenderState}
 * @property {Overlays.OverlayProperties|QUuid} [start] When using {@link Pointers.createPointer}, an optionally defined object
 * to represent the beginning of the Ray Pointer, using the properties you would normally pass to {@link Overlays.addOverlay},
 * plus the type (as a <code>type</code> field). When returned from {@link Pointers.getPointerProperties}, the ID of the created
 * object if it exists, or a null ID otherwise.
 * @property {Overlays.OverlayProperties|QUuid} [path] When using {@link Pointers.createPointer}, an optionally defined object
 * to represent the path of the Ray Pointer, using the properties you would normally pass to {@link Overlays.addOverlay}, plus
 * the type (as a <code>type</code> field), which <b>must</b> be <code>"line3d"</code>. When returned from {@link
 * Pointers.getPointerProperties}, the ID of the created object if it exists, or a null ID otherwise.
 * @property {Overlays.OverlayProperties|QUuid} [end] When using {@link Pointers.createPointer}, an optionally defined object to
 * represent the end of the Ray Pointer, using the properties you would normally pass to {@link Overlays.addOverlay}, plus the
 * type (as a <code>type</code> field). When returned from {@link Pointers.getPointerProperties}, the ID of the created object
 * if it exists, or a null ID otherwise.
 */
/**jsdoc
 * A set of properties that can be passed to {@link Pointers.createPointer} to create a new Pointer. Contains the relevant
 * {@link Picks.PickProperties} to define the underlying Pick.
 ...
 * @property {boolean} [lockEnd=false] If true, the end of the Pointer will lock on to the center of the object at which the
 * pointer is pointing.
 * @property {boolean} [distanceScaleEnd=false] If true, the dimensions of the end of the Pointer will scale linearly with
 * distance.
 * @property {boolean} [scaleWithParent=false] If true, the width of the Pointer's path will scale linearly with the pick
 * parent's scale. scaleWithAvatar is an alias but is deprecated.
 * @property {boolean} [followNormal=false] If true, the end of the Pointer will rotate to follow the normal of the intersected
 * surface.
 * @property {number} [followNormalStrength=0.0] The strength of the interpolation between the real normal and the visual normal
 * if followNormal is true. <code>0-1</code>.  If 0 or 1, the normal will follow exactly.
 * @property {boolean} [enabled=false]
 * @property {Pointers.RayPointerRenderState[]|Object.<string, Pointers.RayPointerRenderState>} [renderStates] A collection of
 * different visual states to switch between. When using {@link Pointers.createPointer}, a list of RayPointerRenderStates. When
 * returned from {@link Pointers.getPointerProperties}, a map between render state names and RayPointRenderStates.
 * @property {Pointers.DefaultRayPointerRenderState[]|Object.<string, Pointers.DefaultRayPointerRenderState>}
 * [defaultRenderStates] A collection of different visual states to use if there is no intersection. When using {@link
 * Pointers.createPointer}, a list of DefaultRayPointerRenderStates. When returned from {@link Pointers.getPointerProperties}, a
 * map between render state names and DefaultRayPointRenderStates.
 * @property {boolean} [hover=false] If this Pointer should generate hover events.
 * @property {Pointers.Trigger[]} [triggers] A list of different triggers mechanisms that control this Pointer's click event
 * generation.
 */
unsigned int PointerScriptingInterface::createLaserPointer(const QVariant& properties) const {
        ...
        for (const QVariant& renderStateVariant : renderStateVariants) {
            if (renderStateVariant.isValid()) {
                ...
                if (renderStateMap["name"].isValid() && renderStateMap["distance"].isValid()) {
                    ...
                    defaultRenderStates[name] = std::pair<float, std::shared_ptr<StartEndRenderState>>(
                        distance, LaserPointer::buildRenderState(renderStateMap));
                }
            }
        }
    }
        ...
        for (const QVariant& triggerVariant : triggerVariants) {
            if (triggerVariant.isValid()) {
                ...
                if (triggerMap["action"].isValid() && triggerMap["button"].isValid()) {
                    controller::Endpoint::Pointer endpoint = userInputMapper->endpointFor(
                        controller::Input(triggerMap["action"].toUInt()));
                    if (endpoint) {
    ...
    return DependencyManager::get<PointerManager>()->addPointer(
        std::make_shared<LaserPointer>(properties, renderStates, defaultRenderStates, hover, triggers, faceAvatar, followNormal,
                                       followNormalStrength, centerEndY, lockEnd, distanceScaleEnd, scaleWithParent, enabled));
}
 ...
 */
/**jsdoc
 * A set of properties used to define the visual aspect of a Parabola Pointer in the case that the Pointer is not intersecting
 * something.  Same as a {@link Pointers.ParabolaPointerRenderState}, but with an additional distance field.
 *
 * @typedef {object} Pointers.DefaultParabolaPointerRenderState
 * @augments Pointers.ParabolaPointerRenderState
 * @property {number} distance The distance along the parabola at which to render the end of this Parabola Pointer, if one is
 * defined.
 */
/**jsdoc
 * A set of properties used to define the visual aspect of a Parabola Pointer in the case that the Pointer is intersecting
 * something.
 *
 ...
 * Pointers.setRenderState} and {@link Pointers.editRenderState}
 * @property {Overlays.OverlayProperties|QUuid} [start] When using {@link Pointers.createPointer}, an optionally defined object
 * to represent the beginning of the Parabola Pointer, using the properties you would normally pass to {@link
 * Overlays.addOverlay}, plus the type (as a <code>type</code> field). When returned from {@link Pointers.getPointerProperties},
 * the ID of the created object if it exists, or a null ID otherwise.
 * @property {Pointers.ParabolaProperties} [path]  When using {@link Pointers.createPointer}, the optionally defined rendering
 * properties of the parabolic path defined by the Parabola Pointer. Not defined in {@link Pointers.getPointerProperties}.
 * @property {Overlays.OverlayProperties|QUuid} [end] When using {@link Pointers.createPointer}, an optionally defined object to
 * represent the end of the Parabola Pointer, using the properties you would normally pass to {@link Overlays.addOverlay}, plus
 * the type (as a <code>type</code> field). When returned from {@link Pointers.getPointerProperties}, the ID of the created
 * object if it exists, or a null ID otherwise.
 */
/**jsdoc
 * A set of properties that can be passed to {@link Pointers.createPointer} to create a new Pointer. Contains the relevant
 ...
 * @property {boolean} [distanceScaleEnd=false] If true, the dimensions of the end of the Pointer will scale linearly with
 * distance.
 * @property {boolean} [scaleWithParent=true] If true, the width of the Pointer's path will scale linearly with the pick
 * parent's scale. scaleWithAvatar is an alias but is deprecated.
 * @property {boolean} [followNormal=false] If true, the end of the Pointer will rotate to follow the normal of the intersected
 * surface.
 * @property {number} [followNormalStrength=0.0] The strength of the interpolation between the real normal and the visual normal
 * if followNormal is true. <code>0-1</code>.  If 0 or 1, the normal will follow exactly.
 * @property {boolean} [enabled=false]
 * @property {Pointers.ParabolaPointerRenderState[]|Object.<string, Pointers.ParabolaPointerRenderState>} [renderStates] A
 * collection of different visual states to switch between. When using {@link Pointers.createPointer}, a list of
 * ParabolaPointerRenderStates. When returned from {@link Pointers.getPointerProperties}, a map between render state names and
 * ParabolaPointerRenderStates.
 * @property {Pointers.DefaultParabolaPointerRenderState[]|Object.<string, Pointers.DefaultParabolaPointerRenderState>}
 * [defaultRenderStates] A collection of different visual states to use if there is no intersection. When using {@link
 * Pointers.createPointer}, a list of DefaultParabolaPointerRenderStates. When returned from {@link
 * Pointers.getPointerProperties}, a map between render state names and DefaultParabolaPointerRenderStates.
 * @property {boolean} [hover=false] If this Pointer should generate hover events.
 * @property {Pointers.Trigger[]} [triggers] A list of different triggers mechanisms that control this Pointer's click event
 * generation.
 */
unsigned int PointerScriptingInterface::createParabolaPointer(const QVariant& properties) const {
        ...
        for (const QVariant& renderStateVariant : renderStateVariants) {
            if (renderStateVariant.isValid()) {
                ...
                if (renderStateMap["name"].isValid() && renderStateMap["distance"].isValid()) {
                    ...
                    float distance = renderStateMap["distance"].toFloat();
                    defaultRenderStates[name] = std::pair<float, std::shared_ptr<StartEndRenderState>>(
                        distance, ParabolaPointer::buildRenderState(renderStateMap));
                }
            }
        }
    }
    ...
    return DependencyManager::get<PointerManager>()->addPointer(
        std::make_shared<ParabolaPointer>(properties, renderStates, defaultRenderStates, hover, triggers, faceAvatar,
                                          followNormal, followNormalStrength, centerEndY, lockEnd, distanceScaleEnd,
                                          scaleWithParent, enabled));
}

void PointerScriptingInterface::editRenderState(unsigned int uid, const QString& renderState,
