...
bool OculusLegacyDisplayPlugin::internalActivate() {
    ...
    _cullingProjection = toGlm(
        ovrMatrix4f_Projection(combinedFov, DEFAULT_NEAR_CLIP, DEFAULT_FAR_CLIP, ovrProjection_RightHanded));

    _renderTargetSize = uvec2(eyeSizes[0].x + eyeSizes[1].x, std::max(eyeSizes[0].y, eyeSizes[1].y));

    if (!ovrHmd_ConfigureTracking(_hmd, ovrTrackingCap_Orientation | ovrTrackingCap_Position | ovrTrackingCap_MagYawCorrection,
                                  0)) {
...
void OculusLegacyDisplayPlugin::customizeContext() {
    ...
    ovrGLConfig config;
    memset(&config, 0, sizeof(ovrRenderAPIConfig));
...
void OculusLegacyDisplayPlugin::hmdPresent() {
    ...
    if (_hmdWindow->makeCurrent()) {
        ...
        glDeleteSync(sync);
        ovr_for_each_eye([&](ovrEyeType eye) { reinterpret_cast<ovrGLTexture&>(_eyeTextures[eye]).OGL.TexId = texture; });
