...
HTTPManager::HTTPManager(const QHostAddress& listenAddress, quint16 port, const QString& documentRoot,
...
bool HTTPManager::handleHTTPRequest(HTTPConnection* connection, const QUrl& url, bool skipSubHandler) {
    ...
    if (!_documentRoot.isEmpty()) {
        ...
        if (subPath.endsWith('/') || subPath.size() == 0) {
            QStringList possibleIndexFiles = QStringList() << "index.html"
                                                           << "index.shtml";
        ...
        if (!filePath.isEmpty()) {
            ...
            if (localFileInfo.completeSuffix() == "shtml") {
                ...
                while ((matchPosition = includeRegExp.indexIn(localFileString, matchPosition)) != -1) {
                    ...
                    QString includeFilePath = isFileInclude ? localFileInfo.canonicalPath() + "/" + includeRegExp.cap(2)
            ...
            auto mimeType = localFileInfo.suffix() == "shtml" ? QString { "text/html" }
