...
void glbackend_glDrawElementsInstancedBaseVertexBaseInstance(GLenum mode, GLsizei count, GLenum type, const GLvoid* indices,
...
void GLESBackend::do_drawIndexedInstanced(const Batch& batch, size_t paramOffset) {
    ...
    if (isStereo()) {
...
#ifdef GPU_STEREO_DRAWCALL_INSTANCED
        glbackend_glDrawElementsInstancedBaseVertexBaseInstance(mode, numIndices, glType, indexBufferByteOffset,
                                                                trueNumInstances, 0, startInstance);
#else
        setupStereoSide(0);
        glbackend_glDrawElementsInstancedBaseVertexBaseInstance(mode, numIndices, glType, indexBufferByteOffset, numInstances,
                                                                0, startInstance);
        setupStereoSide(1);
        glbackend_glDrawElementsInstancedBaseVertexBaseInstance(mode, numIndices, glType, indexBufferByteOffset, numInstances,
                                                                0, startInstance);
        ...
        // qDebug() << "GLESBackend::do_drawIndexedInstanced glbackend_glDrawElementsInstancedBaseVertexBaseInstance " <<
        // numInstances;
        glbackend_glDrawElementsInstancedBaseVertexBaseInstance(mode, numIndices, glType, indexBufferByteOffset, numInstances,
                                                                0, startInstance);
