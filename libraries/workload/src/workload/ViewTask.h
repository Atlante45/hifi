...
const std::vector<glm::vec2> MIN_VIEW_BACK_FRONTS = { { 3.0f, 4.0f }, { 6.0f, 8.0f }, { 9.0f, 12.0f } };

const std::vector<glm::vec2> MAX_VIEW_BACK_FRONTS = { { 100.0f, 1600.0f }, { 150.0f, 10000.0f }, { 250.0f, 16000.0f } };
...
public:
    ...
    void setR1Front(float d) {
        data.r1Front = d;
        emit dirty();
    }
    void setR1Back(float d) {
        data.r1Back = d;
        emit dirty();
    }
    void setR2Front(float d) {
        data.r2Front = d;
        emit dirty();
    }
    void setR2Back(float d) {
        data.r2Back = d;
        emit dirty();
    }
    void setR3Front(float d) {
        data.r3Front = d;
        emit dirty();
    }
    void setR3Back(float d) {
        data.r3Back = d;
        emit dirty();
    }

    bool getFreezeView() const { return data.freezeViews; }
    void setFreezeView(bool freeze) {
        data.freezeViews = freeze;
        emit dirty();
    }
    bool useAvatarView() const { return data.useAvatarView; }
    void setUseAvatarView(bool use) {
        data.useAvatarView = use;
        emit dirty();
    }
    bool forceViewHorizontal() const { return data.forceViewHorizontal; }
    void setForceViewHorizontal(bool use) {
        data.forceViewHorizontal = use;
        emit dirty();
    }

    bool simulateSecondaryCamera() const { return data.simulateSecondaryCamera; }
    void setSimulateSecondaryCamera(bool use) {
        data.simulateSecondaryCamera = use;
        emit dirty();
    }
...
public:
    ...
    void setRegulateViewRanges(bool use) {
        data.regulateViewRanges = use;
        emit dirty();
    }
...
struct Regulator {
    ...
    Regulator() {}
    Regulator(const Timing_ns& budget_ns, const glm::vec2& minRange, const glm::vec2& maxRange,
              const glm::vec2& relativeStepDown, const glm::vec2& relativeStepUp) :
        _minRange(minRange),
        _maxRange(maxRange),
