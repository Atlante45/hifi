...
std::unique_ptr<SendQueue> SendQueue::create(Socket* socket, HifiSockAddr destination, SequenceNumber currentSequenceNumber,
    ...
    auto queue = std::unique_ptr<SendQueue>(
        new SendQueue(socket, destination, currentSequenceNumber, currentMessageNumber, hasReceivedHandshakeACK));
    ...
    while (true) {
        ...
        if (!_naks.isEmpty()) {
            ...
            if (it != _sentPackets.end()) {
                ...
                emit packetRetransmitted(wireSize, payloadSize, sequenceNumber, p_high_resolution_clock::now());
        ...
        if (locker.owns_lock() && (_packets.isEmpty() || isFlowWindowFull()) && _naks.isEmpty()) {
            ...
            if (uint32_t(_lastACKSequenceNumber) == uint32_t(_currentSequenceNumber)) {
                ...
                if ((cvStatus == std::cv_status::timeout || (now - _lastPacketSentAt > estimatedTimeout)) &&
                    (_packets.isEmpty() || isFlowWindowFull()) && _naks.isEmpty() &&
