...
void NodeList::sendDomainServerCheckIn() {
    ...
    if (_publicSockAddr.isNull()) {
        ...
        PacketType domainPacketType = !_domainHandler.isConnected() ? PacketType::DomainConnectRequest
...
void NodeList::sendAssignment(Assignment& assignment) {
    PacketType assignmentPacketType = assignment.getCommand() == Assignment::CreateCommand ? PacketType::CreateAssignment
    ...
    if (!nodeID.isNull() && getSessionUUID() != nodeID) {
        eachMatchingNode(
            ...
            [&nodeID, ignoreEnabled, this](const SharedNodePointer& destinationNode) {
                ...
                auto ignorePacket = NLPacket::create(PacketType::NodeIgnoreRequest, NUM_BYTES_RFC4122_UUID + sizeof(bool),
                                                     true);
    ...
    if (!nodeID.isNull() && getSessionUUID() != nodeID) {
        ...
        if (audioMixer) {
            if (isIgnoringNode(nodeID)) {
                ...
                // setup the packet
                auto personalMutePacket = NLPacket::create(PacketType::NodeIgnoreRequest, NUM_BYTES_RFC4122_UUID + sizeof(bool),
                                                           true);
...
void NodeList::setAvatarGain(const QUuid& nodeID, float gain) {
    ...
    if (getSessionUUID() != nodeID) {
        ...
        if (audioMixer) {
            // setup the packet
            auto setAvatarGainPacket = NLPacket::create(PacketType::PerAvatarGainSet, NUM_BYTES_RFC4122_UUID + sizeof(float),
                                                        true);
