...
ScriptableResource* ResourceCache::prefetch(const QUrl& url, void* extra, size_t extraHash) {
    ...
    if (QThread::currentThread() != thread()) {
        ...
        BLOCKING_INVOKE_METHOD(this, "prefetch", Q_RETURN_ARG(ScriptableResource*, result), Q_ARG(QUrl, url),
                               Q_ARG(void*, extra), Q_ARG(size_t, extraHash));
        return result;
    }
...
QSharedPointer<Resource> ResourceCache::getResource(const QUrl& url, const QUrl& fallback, void* extra, size_t extraHash) {
    QSharedPointer<Resource> resource;
    {
        ...
        if (resourcesWithExtraHashIter != resourcesWithExtraHash.end()) {
            ...
            // We haven't seen this extra info before, but we've already downloaded the resource.  We need a new copy of this
            // object (with any old hash).
...
bool Resource::handleFailedRequest(ResourceRequest::Result result) {
    ...
    switch (result) {
        ...
        case ResourceRequest::Result::ServerUnavailable: {
            ...
            qCDebug(networking) << "Retryable error while loading: attempt:" << _attempts
