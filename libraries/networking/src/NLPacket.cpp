...
NLPacket::NLPacket(Packet&& packet) : Packet(std::move(packet)) {
    readType();
    readVersion();
    readSourceID();
...
NLPacket::NLPacket(NLPacket&& other) : Packet(std::move(other)) {
    _type = other._type;
    _version = other._version;
...
void NLPacket::setType(PacketType type) {
    ...
    Q_ASSERT(NLPacket::totalHeaderSize(_type, isPartOfMessage()) == NLPacket::totalHeaderSize(type, isPartOfMessage()));
...
void NLPacket::writeVerificationHash(HMACAuth& hmacAuth) const {
    ...
    auto offset = Packet::totalHeaderSize(isPartOfMessage()) + sizeof(PacketType) + sizeof(PacketVersion) + NUM_BYTES_LOCALID;
