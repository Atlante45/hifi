...
NetworkTexture::NetworkTexture(const QUrl& url) : Resource(url), _type(), _maxNumPixels(100) {
...
public:
    ImageReader(const QWeakPointer<Resource>& resource, const QUrl& url, const QByteArray& data, int maxNumPixels);
...
#endif
        auto target = getBackendTarget();
        texture = image::processImage(std::move(buffer), _url.toString().toStdString(), _maxNumPixels,
