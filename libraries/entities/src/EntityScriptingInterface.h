...
public slots:
    ...
    Q_INVOKABLE RayToEntityIntersectionResult findRayIntersectionBlocking(
        const PickRay& ray, bool precisionPicking = false, const QScriptValue& entityIdsToInclude = QScriptValue(),
        const QScriptValue& entityIdsToDiscard = QScriptValue());
    ...
    /**jsdoc
     * Get the IDs of entities, overlays, and avatars that are directly parented to an entity, overlay, or avatar model. Recurse
     * on the IDs returned by the function to get all descendants of an entity, overlay, or avatar.
     ...
     *     entity, overlay, or avatar. Does not include children's children, etc. The array is empty if no children can be found
     * or <code>parentID</code> cannot be found.
