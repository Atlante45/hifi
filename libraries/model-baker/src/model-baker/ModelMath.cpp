...
void calculateNormals(const hfm::Mesh& mesh, NormalAccessor normalAccessor, VertexSetter vertexSetter) {
    ...
    for (const HFMMeshPart& part : mesh.parts) {
        for (int i = 0; i < part.quadIndices.size(); i += 4) {
            ...
            glm::vec3 vertices[3]; // Assume all vertices in this quad are in the same plane, so only the first three are needed
        ...
        if ((part.triangleIndices.size() % 3) != 0) {
            HIFI_FCDEBUG_ID(model_baker(), repeatMessageID,
                            "Error in baker::calculateNormals: part.triangleIndices.size() is not divisible by three");
        }
    }
}
...
void calculateTangents(const hfm::Mesh& mesh, IndexAccessor accessor) {
    ...
    for (const HFMMeshPart& part : mesh.parts) {
        ...
        if ((part.triangleIndices.size() % 3) != 0) {
            HIFI_FCDEBUG_ID(model_baker(), repeatMessageID,
                            "Error in baker::calculateTangents: part.triangleIndices.size() is not divisible by three");
        }
    }
}
} // namespace baker
