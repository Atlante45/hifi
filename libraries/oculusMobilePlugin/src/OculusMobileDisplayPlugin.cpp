...
bool OculusMobileDisplayPlugin::internalActivate() {
    ...
    _cullingProjection = ovr::toGlm(
        ovrMatrix4f_CreateProjectionFov(90.0f, 90.0f, 0.0f, 0.0f, DEFAULT_NEAR_CLIP, DEFAULT_FAR_CLIP));
    ...
    ovr::for_each_eye([&](ovrEye eye) { _eyeProjections[eye] = _cullingProjection; });
...
void OculusMobileDisplayPlugin::resetSensors() {
    VrHandler::withOvrMobile([&](ovrMobile* session) { vrapi_RecenterPose(session); });
    _currentRenderFrameInfo.renderPose = glm::mat4(); // identity
}

float OculusMobileDisplayPlugin::getTargetFrameRate() const {
    float result = 0.0f;
    VrHandler::withOvrJava(
        [&](const ovrJava* java) { result = vrapi_GetSystemPropertyFloat(java, VRAPI_SYS_PROP_DISPLAY_REFRESH_RATE); });
    return result;
}
...
bool OculusMobileDisplayPlugin::isHmdMounted() const {
    bool result = false;
    VrHandler::withOvrJava(
        [&](const ovrJava* java) { result = VRAPI_FALSE != vrapi_GetSystemStatusInt(java, VRAPI_SYS_STATUS_MOUNTED); });
    return result;
}
