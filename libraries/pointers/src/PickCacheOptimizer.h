...
public:
    QVector4D update(std::unordered_map<uint32_t, std::shared_ptr<PickQuery>>& picks, uint32_t& nextToUpdate, uint64_t expiry,
                     bool shouldPickHUD);
...
template<typename T>
QVector4D PickCacheOptimizer<T>::update(std::unordered_map<uint32_t, std::shared_ptr<PickQuery>>& picks, uint32_t& nextToUpdate,
    ...
    while (numUpdates < picks.size()) {
        ...
        if (!pick->isEnabled() || pick->getFilter().doesPickNothing() || pick->getMaxDistance() < 0.0f || !mathematicalPick) {
            ...
            if (pick->getFilter().doesPickOverlays()) {
                PickCacheKey overlayKey = { pick->getFilter().getOverlayFlags(), pick->getIncludeItems(),
                                            pick->getIgnoreItems() };
