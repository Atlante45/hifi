...
Pose::Pose(const vec3& translation, const quat& rotation, const vec3& velocity, const vec3& angularVelocity) :
...
bool Pose::operator==(const Pose& right) const {
    ...
    return translation == right.getTranslation() && rotation == right.getRotation() && velocity == right.getVelocity() &&
           angularVelocity == right.getAngularVelocity();
}
...
void Pose::fromScriptValue(const QScriptValue& object, Pose& pose) {
    ...
    if (translation.isValid() && rotation.isValid() && velocity.isValid() && angularVelocity.isValid()) {
...
Pose Pose::transform(const glm::mat4& mat) const {
    ...
    Pose pose(transformPoint(mat, translation), rot * rotation, transformVectorFast(mat, velocity), rot * angularVelocity);
    pose.valid = valid;
    return pose;
}
...
Pose Pose::postTransform(const glm::mat4& mat) const {
    ...
    auto velocityOut = velocity +
                       glm::cross(angularVelocity, translationOut - translation); // warning: this may be completely wrong
    auto angularVelocityOut = angularVelocity;

    Pose pose(translationOut, rotationOut, velocityOut, angularVelocityOut);
