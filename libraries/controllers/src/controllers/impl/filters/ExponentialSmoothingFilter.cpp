...
Pose ExponentialSmoothingFilter::apply(Pose value) const {
    if (value.isValid()) {
        ...
        if (_prevSensorValue.isValid()) {
            ...
            sensorValue.translation = _translationConstant * sensorValue.getTranslation() +
                                      (1.0f - _translationConstant) * _prevSensorValue.getTranslation();
            sensorValue.rotation = safeMix(sensorValue.getRotation(), _prevSensorValue.getRotation(),
                                           (1.0f - _rotationConstant));
