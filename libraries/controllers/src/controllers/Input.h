...
enum class ChannelType { UNKNOWN = 0, BUTTON, AXIS, POSE, RUMBLE, INVALID = 0x7 };
...
struct Input {
    ...
    explicit Input(uint16_t device, uint16_t channel, ChannelType type) :
        device(device),
        channel(channel),
        type(uint16_t(type)),
        padding(0) {}
    ...
    bool operator==(const Input& right) const {
        return INVALID_INPUT.id != id && INVALID_INPUT.id != right.id && id == right.id;
    }
