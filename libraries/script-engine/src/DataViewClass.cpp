...
DataViewClass::DataViewClass(ScriptEngine* scriptEngine) : ArrayBufferViewClass(scriptEngine) {
    ...
    // build prototype
    _proto = engine()->newQObject(new DataViewPrototype(this), QScriptEngine::QtOwnership,
...
QScriptValue DataViewClass::construct(QScriptContext* context, QScriptEngine* engine) {
    ...
    if (byteOffsetArg.isNumber() && (byteOffsetArg.toInt32() < 0 || byteOffsetArg.toInt32() > arrayBuffer->size())) {
        engine->evaluate("throw \"RangeError: byteOffset out of range\"");
        return QScriptValue();
    }
    if (byteLengthArg.isNumber() &&
        (byteLengthArg.toInt32() < 0 || byteOffsetArg.toInt32() + byteLengthArg.toInt32() > arrayBuffer->size())) {
