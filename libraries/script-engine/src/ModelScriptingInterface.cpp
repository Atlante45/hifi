...
QString ModelScriptingInterface::meshToOBJ(MeshProxyList in) {
    ...
    foreach (const MeshProxy* meshProxy, in) { meshes.append(meshProxy->getMeshPointer()); }
...
QScriptValue ModelScriptingInterface::transformMesh(glm::mat4 transform, MeshProxy* meshProxy) {
    ...
    graphics::MeshPointer result = mesh->map(
...
QScriptValue ModelScriptingInterface::newMesh(const QVector<glm::vec3>& vertices, const QVector<glm::vec3>& normals,
    ...
    gpu::BufferView vertexBufferView(vertexBufferPtr, 0, vertexBufferPtr->getSize(), sizeof(glm::vec3),
    ...
    if (vertices.size() == normals.size()) {
        ...
        gpu::BufferView normalBufferView(normalBufferPtr, 0, normalBufferPtr->getSize(), sizeof(glm::vec3),
                                         gpu::Element(gpu::VEC3, gpu::FLOAT, gpu::XYZ));
