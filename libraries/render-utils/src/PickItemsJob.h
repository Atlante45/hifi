...
public:
    ...
    PickItemsJob(render::ItemKey::Flags validKeys = render::ItemKey::Builder().withTypeMeta().withTypeShape().build()._flags,
                 render::ItemKey::Flags excludeKeys = 0);
...
private:
    ...
    render::ItemBound findNearestItem(const render::RenderContextPointer& renderContext, const render::ItemBounds& inputs,
                                      float& minIsectDistance) const;
};
