...
DeferredFrameTransform::DeferredFrameTransform() {
    ...
    _frameTransformBuffer = gpu::BufferView(
        std::make_shared<gpu::Buffer>(sizeof(FrameTransform), (const gpu::Byte*)&frameTransform));
}
...
void DeferredFrameTransform::update(RenderArgs* args, glm::vec2 jitter) {
    ...
    if (!isStereo) {
        ...
        frameTransformBuffer.invpixelInfo = glm::vec4(1.0f / (float)(args->_viewport.z >> 1), 1.0f / args->_viewport.w, 0.0f,
                                                      0.0f);
    }
}

void GenerateDeferredFrameTransform::run(const render::RenderContextPointer& renderContext, const Input& jitter,
                                         Output& frameTransform) {
    if (!frameTransform) {
