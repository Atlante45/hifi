...
void initZPassPipelines(ShapePlumber& plumber, gpu::StatePointer state, const render::ShapePipeline::BatchSetter& batchSetter,
                        const render::ShapePipeline::ItemSetter& itemSetter);
...
void initZPassPipelines(ShapePlumber& shapePlumber, gpu::StatePointer state,
    ...
    shapePlumber.addPipeline(ShapeKey::Filter::Builder().withoutDeformed().withoutFade(),
                             gpu::Shader::createProgram(model_shadow), state);
    shapePlumber.addPipeline(ShapeKey::Filter::Builder().withoutDeformed().withFade(),
                             gpu::Shader::createProgram(model_shadow_fade), state, extraBatchSetter, itemSetter);

    shapePlumber.addPipeline(ShapeKey::Filter::Builder().withDeformed().withoutDualQuatSkinned().withoutFade(),
                             gpu::Shader::createProgram(deformed_model_shadow), state);
    shapePlumber.addPipeline(ShapeKey::Filter::Builder().withDeformed().withoutDualQuatSkinned().withFade(),
...
void RenderPipelines::updateMultiMaterial(graphics::MultiMaterial& multiMaterial) {
    ...
    std::call_once(once, [] {
        for (int i = 0; i < graphics::Material::NUM_TOTAL_FLAGS; i++) {
            ...
            if (i != graphics::MaterialKey::OPACITY_MASK_MAP_BIT && i != graphics::MaterialKey::OPACITY_TRANSLUCENT_MAP_BIT) {
                allFlags.insert(i);
            }
        }
    });
            ...
            switch (flag) {
                ...
                case graphics::MaterialKey::SCATTERING_MAP_BIT:
                    if (materialKey.isScatteringMap()) {
                        ...
                        if (itr != textureMaps.end()) {
                            if (itr->second->isDefined()) {
                                drawMaterialTextures->setTexture(gr::Texture::MaterialScattering,
                                                                 itr->second->getTextureView());
                                wasSet = true;
                            } else {
                                multiMaterial.setTexturesLoading(true);
                                forceDefault = true;
                ...
                case graphics::MaterialKey::EMISSIVE_MAP_BIT:
                    ...
                    if (materialKey.isEmissiveMap() && !materialKey.isLightmapMap()) {
                        ...
                        if (itr != textureMaps.end()) {
                            if (itr->second->isDefined()) {
                                drawMaterialTextures->setTexture(gr::Texture::MaterialEmissiveLightmap,
                                                                 itr->second->getTextureView());
                                wasSet = true;
                            } else {
                                multiMaterial.setTexturesLoading(true);
