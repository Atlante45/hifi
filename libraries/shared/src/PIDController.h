...
//  See
//  http://www.wetmachine.com/inventing-the-future/mostly-reliable-performance-of-software-processes-by-dynamic-control-of-quality-parameters/
//
...
public:
    ...
    void setHistorySize(QString label = QString(""), int size = 0) {
        _history.reserve(size);
        _history.resize(0);
        _label = label;
    } // non-empty does logging
    ...
    // There are several values that rarely change and might be thought of as "constants", but which do change during tuning,
    // debugging, or other special-but-expected circumstances. Thus the instance vars are not const.
...
protected:
    void reportHistory();
    void updateHistory(float measured, float dt, float error, float accumulatedError, float changeInErro, float p, float i,
                       float d, float computedValue);
