...
#define HIFI_FCDEBUG(category, message)                                                                                        \
    do {                                                                                                                       \
        if (category.isDebugEnabled()) {                                                                                       \
            ...
            LogHandler::getInstance().printRepeatedMessage(repeatedMessageID_, LogDebug,                                       \
                                                           QMessageLogContext(__FILE__, __LINE__, __func__,                    \
...
#define HIFI_FCDEBUG_ID(category, messageID, message)                                                                          \
    do {                                                                                                                       \
        if (category.isDebugEnabled()) {                                                                                       \
            ...
            QDebug debugStringReceiver_(&logString_);                                                                          \
            debugStringReceiver_ << message;                                                                                   \
            LogHandler::getInstance().printRepeatedMessage(                                                                    \
                messageID, LogDebug, QMessageLogContext(__FILE__, __LINE__, __func__, category().categoryName()), logString_); \
        }                                                                                                                      \
    } while (false)
