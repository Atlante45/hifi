...
// When profiling something that may happen many times per frame, use a xxx_detail category so that they may easily be filtered
// out of trace results
...
class Duration {
public:
    Duration(const QLoggingCategory& category, const QString& name, uint32_t argbColor = 0xff0000ff, uint64_t payload = 0,
...
inline void syncBegin(const QLoggingCategory& category, const QString& name, const QString& id,
...
inline void syncEnd(const QLoggingCategory& category, const QString& name, const QString& id,
                    const QVariantMap& args = QVariantMap(), const QVariantMap& extra = QVariantMap()) {
    if (category.isDebugEnabled()) {
...
inline void asyncBegin(const QLoggingCategory& category, const QString& name, const QString& id,
                       const QVariantMap& args = QVariantMap(), const QVariantMap& extra = QVariantMap()) {
    if (category.isDebugEnabled()) {
...
inline void asyncEnd(const QLoggingCategory& category, const QString& name, const QString& id,
                     const QVariantMap& args = QVariantMap(), const QVariantMap& extra = QVariantMap()) {
    if (category.isDebugEnabled()) {
...
inline void instant(const QLoggingCategory& category, const QString& name, const QString& scope = "t",
                    const QVariantMap& args = QVariantMap(), QVariantMap extra = QVariantMap()) {
    if (category.isDebugEnabled()) {
        extra["s"] = scope;
...
inline void counter(const QLoggingCategory& category, const QString& name, const QVariantMap& args,
...
#define PROFILE_RANGE_EX(category, name, argbColor, payload, ...)                                                              \
    Duration profileRangeThis(trace_##category(), name, argbColor, (uint64_t)payload, ##__VA_ARGS__);
#define PROFILE_RANGE_BEGIN(category, rangeId, name, argbColor)                                                                \
    rangeId = Duration::beginRange(trace_##category(), name, argbColor)
...
#define PROFILE_COUNTER_IF_CHANGED(category, name, type, value)                                                                \
    {                                                                                                                          \
        static type lastValue = 0;                                                                                             \
        type newValue = value;                                                                                                 \
        if (newValue != lastValue) {                                                                                           \
            counter(trace_##category(), name, { { name, newValue } });                                                         \
            lastValue = newValue;                                                                                              \
        }                                                                                                                      \
    }
...
#define SAMPLE_PROFILE_RANGE(chance, category, name, ...)                                                                      \
    if (randFloat() <= chance) {                                                                                               \
        PROFILE_RANGE(category, name);                                                                                         \
    }
#define SAMPLE_PROFILE_RANGE_EX(chance, category, name, ...)                                                                   \
    if (randFloat() <= chance) {                                                                                               \
        PROFILE_RANGE_EX(category, name, argbColor, payload, ##__VA_ARGS__);                                                   \
    }
#define SAMPLE_PROFILE_COUNTER(chance, category, name, ...)                                                                    \
    if (randFloat() <= chance) {                                                                                               \
        PROFILE_COUNTER(category, name, ##__VA_ARGS__);                                                                        \
    }
#define SAMPLE_PROFILE_INSTANT(chance, category, name, ...)                                                                    \
    if (randFloat() <= chance) {                                                                                               \
        PROFILE_INSTANT(category, name, ##__VA_ARGS__);                                                                        \
    }
...
#define DETAILED_PROFILE_RANGE_EX(category, name, argbColor, payload, ...)                                                     \
    Duration profileRangeThis(trace_##category(), name, argbColor, (uint64_t)payload, ##__VA_ARGS__);
