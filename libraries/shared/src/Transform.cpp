...
void Transform::evalRotationScale(Quat& rotation, Vec3& scale, const Mat3& rotationScaleMatrix) {
    ...
    do {
        ...
        norm = 0.0;
        for (int i = 0; i < 3; i++) {
            float n = static_cast<float>(fabs(rotationMat[0][i] - nextRotation[0][i]) +
    ...
    Mat3 matRot(rotationScaleMatrix[0] / scale.x, rotationScaleMatrix[1] / scale.y, rotationScaleMatrix[2] / scale.z);
...
QDebug& operator<<(QDebug& debug, const Transform& transform) {
    debug << "Transform, trans = (" << transform._translation.x << transform._translation.y << transform._translation.z
          << "), rot = (" << transform._rotation.x << transform._rotation.y << transform._rotation.z << transform._rotation.w
          << "), scale = (" << transform._scale.x << transform._scale.y << transform._scale.z << ")";
    return debug;
}
