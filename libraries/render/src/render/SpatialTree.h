...
public:
    ...
    public:
        ...
        Coord3 octantAxes(Link octant) const {
            return Coord3((Coord) bool(octant & XAxis), (Coord) bool(octant & YAxis), (Coord) bool(octant & ZAxis));
        }
    ...
    static const Index MAXIMUM_INDEX = INT_MAX; // For fun, test setting this to 100 and this should still works with only the
                                                // allocated maximum number of cells
    ...
    public:
        ...
        Cell() :
            _links({ { INVALID_CELL, INVALID_CELL, INVALID_CELL, INVALID_CELL, INVALID_CELL, INVALID_CELL, INVALID_CELL,
                       INVALID_CELL, INVALID_CELL, INVALID_CELL } }) {}

        Cell(Index parent, Location loc) :
            _links({ { INVALID_CELL, INVALID_CELL, INVALID_CELL, INVALID_CELL, INVALID_CELL, INVALID_CELL, INVALID_CELL,
                       INVALID_CELL, parent, INVALID_CELL } }),
            _location(loc) {}
...
public:
    ...
    glm::vec3 evalPos(const Coord3& coord, float cellWidth) const { return getOrigin() + glm::vec3(coord) * cellWidth; }
    ...
    glm::vec3 clampRelPosToTreeRange(const glm::vec3& pos) const {
        ...
        return glm::vec3(std::min(std::max(pos.x, 0.0f), _size - EPSILON), std::min(std::max(pos.y, 0.0f), _size - EPSILON),
    ...
    int selectCellItems(ItemSelection& selection, const ItemFilter& filter, const ViewFrustum& frustum, float threshold) const;
};
} // namespace render
