...
int AvatarAudioStream::parseStreamProperties(PacketType type, const QByteArray& packetAfterSeqNum, int& numAudioSamples) {
    ...
    if (type == PacketType::SilentAudioFrame) {
        ...
        if (isStereo != _isStereo) {
            _ringBuffer.resizeForFrameSize(isStereo ? AudioConstants::NETWORK_FRAME_SAMPLES_STEREO
            ...
            if (_codec) {
                ...
                _decoder = _codec->createDecoder(AudioConstants::SAMPLE_RATE,
                                                 isStereo ? AudioConstants::STEREO : AudioConstants::MONO);
            }
