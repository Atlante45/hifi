...
void vhacd::VHACDUtil::fattenMesh(const FBXMesh& mesh, const glm::mat4& geometryOffset, FBXMesh& result) const {
    ...
    foreach (const FBXMeshPart& meshPart, mesh.parts) { getTrianglesInMeshPart(meshPart, triangleIndices); }
...
bool vhacd::VHACDUtil::computeVHACD(FBXGeometry& geometry, VHACD::IVHACD::Parameters params, FBXGeometry& result,
    ...
    int numParts = 0;
    foreach (const FBXMesh& mesh, geometry.meshes) { numParts += mesh.parts.size(); }
    if (_verbose) {
